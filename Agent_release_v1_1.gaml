...
global {
	...
	init 
	{
		...
		{
			mySupplies <-one_of(availableSupplies);
			headColor <- mySupplies.color;
			current_cell <- mySupplies.current_cell;
			current_cell.is_free <- false;
			remove current_cell from: free_cell;
			location <- current_cell.location;
...
species requester parent:people {
	...
	reflex handle_reply_from_supplies when: (!empty(informs)) 
	{
		write 'requester '+string(self) +' received a reply from supply station';
...
species deliveryman parent:people {
	...
	init
	{
		write 'deliveryman '+string(self) +' started binding to supply station '+ string(mySupplies);
	...
	reflex travel_home when: empty(requesters) // deliveryRange
	{
		target_cell <- home_cell;
